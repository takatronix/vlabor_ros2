#!/bin/bash
#
# VLAbor Local Entrypoint
# VPN不要、同一ネットワーク内で動作するAll-in-One版
#
...
# ros_tcp_endpoint ワークスペース
if [ -f /ros2_ws/install/setup.bash ]; then
    source /ros2_ws/install/setup.bash
    echo -e "${GREEN}[INFO]${NC} ROS TCP Endpoint 読み込み完了"
fi

# vr_ros2 ワークスペース（マウントされている場合）
if [ -f /ros2_ws/src/vr_ros2/install/setup.bash ]; then
    source /ros2_ws/src/vr_ros2/install/setup.bash
    echo -e "${GREEN}[INFO]${NC} vr_ros2 ワークスペース読み込み完了"
fi
...
echo ""
echo "=========================================="
echo -e "${GREEN}[INFO]${NC} VLAbor Local 起動完了!"
echo "=========================================="
echo ""
echo "接続情報:"
...
if [ -f /ros2_ws/src/vr_ros2/install/setup.bash ]; then
    ...
    ros2 launch unity_robot_control vr_dual_arm_teleop.launch.py \
        ...
        echo -e "${GREEN}[INFO]${NC} Unity TCP EndpointとFoxglove Bridgeは動作中です。"
        # バックグラウンドプロセスを待機
        wait $UNITY_PID
    }
else
